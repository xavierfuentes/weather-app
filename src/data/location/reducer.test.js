...
import { getLocation, getLocation, getLocation } from './actions';
...
it('handles GET_LOCATION_SUCCESS', () => {
  ...
  expect(locationReducer(initialState, getLocation({ coords, timestamp }))).toEqual({
    ...initialState,
    error: null,
    fetchedAt: timestamp,
    fetching: false,
...
it('handles GET_LOCATION_FAILURE', () => {
  const error = 'Error';
  const timestamp = null;
  expect(locationReducer(initialState, getLocation({ error, timestamp }))).toEqual({
    ...initialState,
    error,
    fetchedAt: timestamp,
    fetching: false,
